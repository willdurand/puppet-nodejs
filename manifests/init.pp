...
class nodejs (
  ...
  exec { 'node-install':
    ...
    path    => '/usr/bin:/bin:/usr/sbin:/sbin',
    cwd     => "${::nodejs::install_dir}/node-${node_version}",
    user    => 'root',
    unless  => "test -f ${::nodejs::install_dir}/node-${node_version}/node",
    timeout => 0,
    require => Exec['node-unpack'],
  }

  exec { 'node-symlink-bin':
    command => "ln -s ${::nodejs::install_dir}/node-${node_version}/node ${node_target_dir}/node-${node_version}",
    path    => '/usr/bin:/bin:/usr/sbin:/sbin',
    user    => 'root',
  ...
  if ($with_npm) {
    exec { 'npm-download':
      ...
      cwd     => $::nodejs::install_dir,
      user    => 'root',
      unless  => "test -f ${::nodejs::install_dir}/install.sh",
